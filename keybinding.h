...
#define FOREACH_KEYBINDING(KEYBINDING) \
	KEYBINDING(KEYBINDING_RUN_COMMAND,exec) /* data.c is the string to execute */ \
	...
	KEYBINDING(KEYBINDING_CYCLE_VIEWS,cycle_views)       /* data.b is 0 if forward, 1 if reverse */\
	KEYBINDING(KEYBINDING_CYCLE_TILES,cycle_tiles)       /* data.b is 0 if forward, 1 if reverse */\
	KEYBINDING(KEYBINDING_CYCLE_OUTPUT,cycle_outputs)      /* data.b is 0 if forward, 1 if reverse */\
	KEYBINDING(KEYBINDING_CONFIGURE_OUTPUT,output)  /* data.o_cfg is the desired output */\
\
	KEYBINDING(KEYBINDING_CONFIGURE_MESSAGE,configure_message) /* data.m_cfg is the desired config */\
	KEYBINDING(KEYBINDING_CONFIGURE_INPUT,input)   /* data.i_cfg is the desired input configuration */\
\
	KEYBINDING(KEYBINDING_QUIT,quit)\
	KEYBINDING(KEYBINDING_NOOP,abort)\
	KEYBINDING(KEYBINDING_SWITCH_OUTPUT,screen)          /* data.u is the desired output */\
	KEYBINDING(KEYBINDING_SWITCH_WORKSPACE,workspace)       /* data.u is the desired workspace */\
	KEYBINDING(KEYBINDING_SWITCH_MODE,mode)            /* data.u is the desired mode */\
	KEYBINDING(KEYBINDING_SWITCH_DEFAULT_MODE,setmode)    /* data.u is the desired mode */\
	KEYBINDING(KEYBINDING_RESIZE_TILE_HORIZONTAL,resize_tile_horizontal) /* data.i is the number of pixels to add */\
\
	KEYBINDING(KEYBINDING_RESIZE_TILE_VERTICAL,resize_tile_vertical) /* data.i is the number of pixels to add to */\
\
	KEYBINDING(KEYBINDING_MOVE_VIEW_TO_WORKSPACE,movetoworkspace)    /* data.u is the desired workspace */\
	KEYBINDING(KEYBINDING_MOVE_VIEW_TO_OUTPUT,movetoscreen)       /* data.u is the desired output */\
	KEYBINDING(KEYBINDING_MOVE_VIEW_TO_CYCLE_OUTPUT,move_view_to_cycle_output) /* data.b is 0 if forward, 1 if */\
\
...
\
	KEYBINDING(KEYBINDING_DEFINEKEY,definekey)  /* data.kb is the keybinding definition */\
	KEYBINDING(KEYBINDING_BACKGROUND,background) /* data.color is the background color */\
	KEYBINDING(KEYBINDING_DEFINEMODE,definemode) /* data.c is the mode name */\
	KEYBINDING(KEYBINDING_WORKSPACES,workspaces) /* data.i is the number of workspaces */\
